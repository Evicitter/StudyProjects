//Last update time : 09.10.2010 00:24:21
#include <WINDEF.H>
#include <stdio.h>
#include <intrin.h>
...
static void ByteToDWord(DWORD* Output, ULONG* Input, UINT nLength){
	UINT i=0;
	UINT j=0;
	for (; j < nLength; i++, j += 4)
	{
...
static void DWordToByte(unsigned char* Output, DWORD* Input, UINT nLength ){
	UINT i = 0;
	UINT j = 0;
	for (; j < nLength; i++, j += 4) 
	{
...
inline void MD5Init (void)
{
	__stosd((PDWORD)m_lpszBuffer,0,16);
...
inline void MD5Update (unsigned char *inBuf, unsigned int inLen)
{
	register int i, ii;
	int mdi;
	UINT4 in[16];
...
/*inline void MD5Final (char* cReturnStr)
{
...
inline void Transform(register UINT4 *buf,register UINT4 *in)
{
...
inline void __fastcall GetMD5(unsigned char* pBuf, UINT nLength, ULONG& cR1, ULONG& cR2, ULONG& cR3, ULONG& cR4)
{
	MD5Init();
