...
class TransAppMsg
{
	typedef void (__fastcall *cbTransAppMsg)(void* buffer, unsigned short bufsize_inbyte);

public:
	TransAppMsg();
	virtual ~TransAppMsg();

	virtual bool Host(wchar_t* chanelname, bool bIsLocal = true, bool bIsOnce = false, bool bIsExternalClock = false, unsigned long ITimeOutForReadMSG = 1000ul);
	virtual bool Join(wchar_t* chanelname, bool bIsLocal = true);
